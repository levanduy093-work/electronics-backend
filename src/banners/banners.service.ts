...
export class BannersService {
  ...
  async findAll() {
    return this.bannerModel.find().sort({ order: 1, createdAt: -1 }).lean().exec();
  }

  async update(id: string, dto: UpdateBannerDto) {
    const updated = await this.bannerModel.findByIdAndUpdate(id, dto, { new: true }).exec();
    if (!updated) {
    ...
    try {
      await session.withTransaction(async () => {
        for (const item of dto.items) {
          await this.bannerModel.updateOne({ _id: item.id }, { $set: { order: item.order } }, { session }).exec();
        }
      });
    } finally {
